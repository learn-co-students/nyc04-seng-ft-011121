...
- What is hashing and how does it make password storage more secure?
  - "abc123" -> "hgoifjgiodjglidjkgka.grgjailrjglirjglijl.dfjdlisjf"
  - "def123" -> "gjildjhlidsjljsildjgsjg.hjigjlsdjgsk;ok/dgjsdofkos"
- What are rainbow tables and how can they defeat a password hashing strategy?
  - "a" -> "gijsfigjorspk;odkf;odkso;fksdo;kf;sod/fdaflkfasdasdasdg"
  - "aa" -> gjilsjg;odksd;ofjfdl;fklojhiotejhoprjhok;okeqpojkropekd
  - "abc123" -> "hgoifjgiodjglidjkgka.grgjailrjglirjglijl.dfjdlisjf"
- What is salting? What is a salt? How can it defeat a rainbow tables attack?
  - "abc123" + "cayenne" -> "ihjlfijgiosfkdjfgkldsgkofgko;kgh"
  - "abc123" + "salt" -> "dhiljtiohjtjhpotkogjkrogjkkfesdadfdsf"

### Implemention

0) Uncomment `bcrypt` in your Gemfile
  0.5) Run `bundle` 
1) Whatever model you want to protect with a password, make sure it has a `password_digest:string` column
2) In the models from step 1, write the `has_secure_password` macro

`password_digest` should only be written in the migrations

What `has_secure_password` does:
 - It takes `password` and turns it into `password_digest`
 - Gives us `#authenticate` instance method
